...
class SimpleEnv2:
    ...
    def __init__(self, 
        ...
        args:
            ...
            action_type: str, type of action space, 'eef_pose','delta_joint_angle' or 'joint_angle' #*  动作空间类型，可以是'末端执行器位姿'、'关节角变化'或'关节角'
    ...
    def reset(self, seed = None):
        ...
        #*盘子位置和旋转
        plate_xyz = np.array([0.3, 0.3, 0.8])
        ...
        def obj_xyzs(range_val=0.04):
            x = np.random.uniform(0.3 - range_val, 0.3 + range_val)
            y = np.random.uniform(-0.3 - range_val, -0.3 + range_val)
            z = 1  # 高度固定
        ...
        for obj_name in fish_objects:
            self.env.set_p_base_body(body_name=obj_name, p=obj_xyzs(range_val=0.12))
        ...
        fish_black_init_pose, fish_blue_init_pose, fish_red_init_pose, fish_yb_init_pose, fish_yellow_init_pose, plate_init_pose = self.get_obj_pose()#!修改物体位姿
        self.obj_init_pose = np.concatenate([fish_black_init_pose, fish_blue_init_pose, fish_red_init_pose, fish_yb_init_pose, fish_yellow_init_pose, plate_init_pose],dtype=np.float32)#!修改物体位姿
        for _ in range(100):
            self.step_env()
    ...
    def set_instruction(self, given = None):#*如果给定指令则使用给定指令，否则随机生成指令
        ...
        """
        if given is None:
            obj_candidates = ['black fish', 'blue fish', 'red fish', 'yb fish', 'yellow fish']#!修改物体文本
            obj1 = random.choice(obj_candidates)
            self.instruction = f'Place the {obj1} on the plate.' #*  根据对象名称生成指令，将特定颜色的杯子放在盘子上
            if obj1 == 'black fish':#*设置目标物体
                self.obj_target = 'fish_black_obj'#!修改物体0
            elif obj1 == 'blue fish':
                self.obj_target = 'fish_blue_obj'#!修改物体1
            elif obj1 == 'red fish':
                self.obj_target = 'fish_red_obj'#!修改物体2
            elif obj1 == 'yb fish':
                self.obj_target = 'fish_yb_obj'#!修改物体3
            else:
                self.obj_target = 'fish_yellow_obj'#!修改物体4
        else:
            self.instruction = given
            if 'black fish' in self.instruction:
                self.obj_target = 'fish_black_obj'#!修改物体0
            elif 'blue fish' in self.instruction:
                self.obj_target = 'fish_blue_obj'#!修改物体1
            elif 'red fish' in self.instruction:
                self.obj_target = 'fish_red_obj'#!修改物体2
            elif 'yb fish' in self.instruction:
                self.obj_target = 'fish_yb_obj'#!修改物体3
            elif 'yellow fish' in self.instruction:
                self.obj_target = 'fish_yellow_obj'#!修改物体4
            else:
                raise ValueError('Instruction does not contain a valid object color (black fish, blue fish, red fish, yb fish, yellow fish)')
    ...
    def get_obj_pose(self):
        '''
        returns: 
            p_fish_black: np.array, position of the fish_black
            p_fish_blue: np.array, position of the fish_blue
            p_fish_red: np.array, position of the fish_red
            p_plate: np.array, position of the plate
        '''
        p_fish_black = self.env.get_p_body('fish_black_obj')#!修改物体0位置
        p_fish_blue = self.env.get_p_body('fish_blue_obj')#!修改物体1位置
        p_fish_red = self.env.get_p_body('fish_red_obj')#!修改物体2位置
        p_fish_yb = self.env.get_p_body('fish_yb_obj')#!修改物体3位置
        p_fish_yellow = self.env.get_p_body('fish_yellow_obj')#!修改物体4位置
        p_plate = self.env.get_p_body('body_obj_plate_11')

        return p_fish_black, p_fish_blue, p_fish_red, p_fish_yb, p_fish_yellow, p_plate #!修改物体位置
    
    def set_obj_pose(self, p_fish_black, p_fish_blue, p_fish_red, p_fish_yb, p_fish_yellow, p_plate):
        '''
        ...
        args:
            ...
            p_plate: np.array, position of the plate
        '''
        self.env.set_p_base_body(body_name='fish_black_obj',p=p_fish_black)#!修改物体0
        self.env.set_R_base_body(body_name='fish_black_obj',R=np.eye(3,3))
        self.env.set_p_base_body(body_name='fish_blue_obj',p=p_fish_blue)#!修改物体1
        self.env.set_R_base_body(body_name='fish_blue_obj',R=np.eye(3,3))
        self.env.set_p_base_body(body_name='fish_red_obj',p=p_fish_red)#!修改物体2
        self.env.set_R_base_body(body_name='fish_red_obj',R=np.eye(3,3))
        self.env.set_p_base_body(body_name='fish_yb_obj',p=p_fish_yb)#!修改物体3
        self.env.set_R_base_body(body_name='fish_yb_obj',R=np.eye(3,3))
        self.env.set_p_base_body(body_name='fish_yellow_obj',p=p_fish_yellow)#!修改物体4
        self.env.set_R_base_body(body_name='fish_yellow_obj',R=np.eye(3,3))
